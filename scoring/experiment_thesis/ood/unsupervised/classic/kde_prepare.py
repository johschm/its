...
def default_kde_params() -> Dict[str, Any]:
    return {
        "bandwidth": 0.1,
        "kernel": "gaussian",
        "dtype": "float32",
...
def sample_kde_params(trial: optuna.Trial, train_cache=None, dataset_info=None, architecture=None, **kwargs) -> Dict[str, Any]:
    ...
    if train_cache:
        ...
        if reducer_name not in available_reducers:
            reducer_name = None
    return {
        "bandwidth": float(trial.suggest_float("bandwidth", 1e-3, 1.0, log=True)),
        "kernel": trial.suggest_categorical("kernel", ["gaussian"]),
        "dtype": "float32",
...
def create_kde_problem(params: Dict[str, Any], train_cache, transform_seq, dataset_info, architecture, **kwargs) -> TransformationProblem:
    # 1. Get embeddings (flatten embeddings like KNN)
    ...
    kde_detector = KDEConfidence(
        bandwidth=params.get("bandwidth", 0.1),
        kernel=params.get("kernel", "gaussian"),
        input_transform=None
    )
    device = kwargs.get("device", torch.device("cpu"))
    kde_detector.to(device)
    kde_detector._fit(embeddings_t.to(dtype_map.get(params.get("dtype", "float32"), torch.float32)))
    kde_detector.to(device)
    ...
    conf_split = PredictedSplitConfidence(kde_detector, EnergyConfidence(), mult=False, b=params.get("split_b", 0.0))
...
# Registration
OOD_DEFAULT_PARAM_FACTORIES["kde"] = default_kde_params
OOD_PARAM_SAMPLERS["kde"] = sample_kde_params
OOD_PROBLEM_FACTORIES["kde"] = create_kde_problem